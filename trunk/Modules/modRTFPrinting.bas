Attribute VB_Name = "modRTFPrinting"
   Option Explicit
...
'Public Sub Ticketing(cTranNo As String)
'    Dim oFrm As New frmWYSIWYG
'
'    With oFrm.rtfTicket
'        .LoadFile App.Path & "\Reports\TicketFormat.rtf", rtfRTF
'        If rs.State = 1 Then rs.Close
'        sSQL = "select * from sales where cTranNo = '" & cTranNo & "'"
'        rs.Open sSQL, cn, adOpenStatic, adLockReadOnly
'        .TextRTF = Replace(.TextRTF, "<cTranNo>", rs!cTranNo & "")
'        .TextRTF = Replace(.TextRTF, "<dDateTime1>", TimeValue(rs!dDateTime) & "")
'        .TextRTF = Replace(.TextRTF, "<dDateTime2>", DateValue(rs!dDateTime) & "")
'        .TextRTF = Replace(.TextRTF, "<cType>", rs!cType & "")
'        .TextRTF = Replace(.TextRTF, "<nNetPrice>", Format(rs!nNetPrice, "00.00") & "")
