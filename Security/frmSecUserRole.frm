...
Begin VB.Form frmSecUserRole 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Security - User Role"
   ClientHeight    =   2055
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   7710
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      EndProperty
      TabCaption(0)   =   "Main"
      TabPicture(0)   =   "frmSecUserRole.frx":0442
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "Frame1"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "List"
      TabPicture(1)   =   "frmSecUserRole.frx":045E
   ...
   End
End
Attribute VB_Name = "frmSecUserRole"
...
Public Enum eSecURMode
    Normal
    AddNewEdit
    Find
End Enum
Private Mode As eSecURMode
...
Private Sub Form_Load()
    ...
    oBar.AcessBit Me, GetValueFrTable("AccessLevel", "SEC_ACCESSLEVEL", "RoleID = '" & SecUserRole & "' AND [Module] = 'SROLE'")
...
Public Sub TBNew()
    ...
    If rsHeader.State <> adStateOpen Then
        oRecordset.OpenRecordset rsHeader, "*", "SEC_ROLE", "WHERE 1 = 0 ", True
        SetDataSource
        SetDataField
    Else
...
On Error GoTo ErrHandler
    ...
    'Audit trail
    UpdateLogFile "Sec - User Role", Trim(txtRoleID), IIf(lBoolean, "Inserted", "Updated")
...
On Error GoTo ErrorHandler
    ...
    'Audit trail
    UpdateLogFile "Sec - User Role", Trim(txtRoleID), "Deleted"
...
Private Sub txtRoleID_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = 13 Then
        'Find
        If Mode = Find Then
            ...
            If Trim(txtRoleID) = "" Then
                oRecordset.OpenRecordset rsHeader, "*", "SEC_ROLE", , True
            Else
                oRecordset.OpenRecordset rsHeader, "*", "SEC_ROLE", "WHERE RoleID LIKE '" & Trim(txtRoleID) & "%'", True
            End If
...
Private Sub txtRoleID_LostFocus()
    ...
    If Mode = AddNewEdit Then
        ...
        Else
            ...
            txtRoleID = Trim(txtRoleID)
            If IDExisting(rsHeader, "RoleID", "SEC_ROLE", Trim(rsHeader!RoleID), , True) Then
                MsgBox "Role ID already exists.", vbInformation, "ComUnion"
                txtRoleID.SetFocus
            End If
        End If
    End If
...
Function MandatoryOK() As Boolean
    ...
    If Trim(txtRoleID) = "" Then
        MandatoryOK = False
        MsgBox "Field 'Role ID' is mandatory. Null value is not allowed.", vbInformation, "ComUnion"
        txtRoleID.SetFocus
        Exit Function
    End If
